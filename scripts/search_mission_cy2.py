...
def main():
    ...
    while not rospy.is_shutdown() and cycle_count < cycles_to_run:
        if not has_initialized:
        ...
        if state == 0:
            ...
            cmd.angular.z = 0.0
            pub.publish(cmd)
            rospy.loginfo("Mission cycle %d complete", cycle_count+1)
            
            cycle_count +=1
            if cycle_count < cycles_to_run:
            	state =0
            	has_initialized = False 
            	rospy.loginfo("Restarting mission (cycle %d of %d)",cycle_count+1,cycles_to_run)
            	rospy.sleep(1.0)
            	continue
            else:
            	rospy.loginfo("All cycles done. Stopping node.")
            break
